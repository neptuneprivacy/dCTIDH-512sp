...
fn main() {
    ...
    // Copy ctidh_api512.o (provides secsidh_keygen, secsidh_derive)
    let api_o = gensteps.join("ctidh_api512.o");
    if !api_o.exists() {
        panic!("missing ctidh_api512.o - run make ctidh-512.main in genSteps");
    }
    ...
    // Rerun if genSteps sources change (optional; can remove for faster rebuilds)
    println!("cargo:rerun-if-changed={}/ctidh_api.c", gensteps.display());
    println!("cargo:rerun-if-changed={}/ctidh_api512.o", gensteps.display());
}
