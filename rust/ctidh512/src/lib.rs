//! # ctidh512 – Rust FFI for dCTIDH 512-bit
//!
//! CTIDH (Commutative Supersingular Isogeny Diffie–Hellman) key exchange,
//!
//! ## Usage
//!
//! ```ignore
...
/// Secret key size in bytes (512-bit parameter set).
pub const SK_SIZE: usize = 74;
/// Public key size in bytes (512-bit parameter set).
pub const PK_SIZE: usize = 64;
/// Shared secret size in bytes (512-bit parameter set).
pub const SS_SIZE: usize = 64;
...
///
/// ```
/// use ctidh512::{keygen, PublicKey, SecretKey};
...
///
/// ```
/// use ctidh512::{keygen, derive, PublicKey, SecretKey};
/// let (pk_a, sk_a) = keygen();
/// let (pk_b, sk_b) = keygen();
